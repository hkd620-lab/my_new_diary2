...
function Library() {
    ...
    const [filter, setFilter] = (0, react_1.useState)("today");
    ...
    (0, react_1.useEffect)(() => {
        async function fetch() {
            ...
            if (!user)
                return;
            try {
                const q = (0, firestore_1.query)((0, firestore_1.collection)(firebase_1.db, "records"), (0, firestore_1.where)("uid", "==", user.uid), (0, firestore_1.orderBy)("createdAt", "desc"));
    ...
    return (<div style={{ padding: 20, maxWidth: 420, margin: "0 auto" }}>
      <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
        ...
        <button onClick={() => setFilter("yesterday")} style={{
            flex: 1,
            padding: "12px 0",
      ...
      {finalRecords.length === 0 && (<div style={{ textAlign: "center", padding: 40, color: "#999" }}>
          이 날의 기록이 없습니다.
        </div>)}

      {finalRecords.map((r) => (<div key={r.id} style={{
                marginBottom: 20,
                padding: 16,
                background: "#fff",
                borderRadius: 12,
          ...
          {r.sections &&
                Object.entries(r.sections).map(([k, v]) => (<div key={k} style={{ marginBottom: 10, fontSize: 14 }}>
                <strong>{k}</strong>: {v}
              </div>))}
          
          <button onClick={() => navigate("/sayu")} style={{
                marginTop: 12,
                width: "100%",
                padding: "10px 0",
                background: "#2C3E50",
